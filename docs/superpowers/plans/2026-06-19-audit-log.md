...
|---|---|---|
| `src/db/migrations/0002_audit_log.sql` | Migration: cria tabela `audit_log` + índices | Criar |
...
**Files:**
- Create: `src/db/migrations/0002_audit_log.sql`

- [ ] **Step 1.1: Criar arquivo de migration**

Criar `src/db/migrations/0002_audit_log.sql`:
...
Esperado: terminal imprime `APPLIED 0001_init.sql` e `APPLIED 0002_audit_log.sql`. Tabela `audit_log` existe no DB.
...
```bash
git add src/db/migrations/0002_audit_log.sql
...
Esperado: PASS com 0 testes (suite vazia). Confirma que a migration `0002` rodou e helpers compilam.
